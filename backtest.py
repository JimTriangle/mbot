...
class BacktestEngine:
    ...
    def __init__(
        ...
        # Strategy instance
        self.strategy = ThreeSwingsStrategy(symbol, interval)
    ...
    def run_backtest(
        ...
        # Reset strategy
        self.strategy = ThreeSwingsStrategy(self.symbol, self.interval)
        ...
        for i, candle in enumerate(candles):
            ...
            if len(self.strategy.pivot_highs) > 0:
                last_high = self.strategy.pivot_highs[-1]
                # Check if this is a newly confirmed pivot
                if not any(p['timestamp'] == last_high['ts'] and p['type'] == 'HIGH' for p in self.pivot_history):
                    self._record_pivot('HIGH', last_high['price'], last_high['ts'], i)

            if len(self.strategy.pivot_lows) > 0:
                last_low = self.strategy.pivot_lows[-1]
                if not any(p['timestamp'] == last_low['ts'] and p['type'] == 'LOW' for p in self.pivot_history):
                    self._record_pivot('LOW', last_low['price'], last_low['ts'], i)
